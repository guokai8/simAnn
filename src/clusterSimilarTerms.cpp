  ...
  for (int i = 0; i < terms_vec.size(); i++) {
    ...
    if (!similarIndices.empty()) {
      ...
      if (!parentFound) {
        ...
        int bestId = -1;
        bool foundNonNA = false;
        int fallbackID = (allTermIds.empty() ? -1 : allTermIds[0]);
        for (int tid : allTermIds) {
          if (tid > 0 && tid <= icScores.size()) {
            double ic = icScores[tid - 1];
            // If ic is NA, skip it in the comparison
            if (!NumericVector::is_na(ic)) {
              // Found a non-NA IC
              foundNonNA = true;
              if (ic < minIC) {
                minIC = ic;
        ...
        // If we never found any non-NA IC, pick the first TID in allTermIds
        if (!foundNonNA) {
          bestId = fallbackID;
        }
        parentTermId = bestId;
